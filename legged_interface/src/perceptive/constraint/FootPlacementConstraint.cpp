//
// Foot placement constraint on perceptive convex terrain regions.
//
...
vector_t FootPlacementConstraint::getValue(scalar_t /*time*/, const vector_t& state, const PreComputation& preComp) const {
  const auto& params = cast<PerceptiveLeggedPrecomputation>(preComp).getFootPlacementConParameters()[contactPointIndex_];
  if (params.a.rows() == 0) {
    return vector_t::Zero(numVertices_);
  }
  return params.a * endEffectorKinematicsPtr_->getPosition(state).front() + params.b;
}
...
VectorFunctionLinearApproximation FootPlacementConstraint::getLinearApproximation(scalar_t /*time*/, const vector_t& state,
  ...
  approx.f = params.a * positionApprox.f + params.b;
  approx.dfdx = params.a * positionApprox.dfdx;
  return approx;
}

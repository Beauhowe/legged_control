...
void PerceptiveLeggedInterface::setupOptimalControlProblem(const std::string& taskFile, const std::string& urdfFile,
  ...
  scalar_t cbfLambda = 1.0;
  RelaxedBarrierPenalty::Config placementCBFPenaltyConfig;
  RelaxedBarrierPenalty::Config placementPenaltyConfig;
  std::tie(enableFootPlacement, enableFootPlacementCBF, cbfLambda, placementCBFPenaltyConfig, placementPenaltyConfig) =
    ...
    if (enableFootPlacement) {
      ...
      problemPtr_->stateSoftConstraintPtr->add(
          ...
          std::unique_ptr<StateCost>(
              new StateSoftConstraint(std::move(footPlacementConstraint),
                                      std::make_unique<RelaxedBarrierPenalty>(placementPenaltyConfig))));
    }

    if (enableFootPlacementCBF) {
      std::unique_ptr<FootPlacementConstraintCBF> footPlacementConstraintCBF(
          new FootPlacementConstraintCBF(*referenceManagerPtr_, *eeKinematicsPtr, i, numVertices_, cbfLambda));
      problemPtr_->softConstraintPtr->add(
          footName + "_footPlacementCBF",
          std::unique_ptr<StateInputCost>(new StateInputSoftConstraint(
              std::move(footPlacementConstraintCBF), std::make_unique<RelaxedBarrierPenalty>(placementCBFPenaltyConfig))));
    }
...
void PerceptiveLeggedInterface::setupReferenceManager(const std::string& taskFile, const std::string&, const std::string& referenceFile,
                                                      bool verbose) {
...
std::tuple<bool, bool, scalar_t, RelaxedBarrierPenalty::Config, RelaxedBarrierPenalty::Config>
PerceptiveLeggedInterface::loadFootPlacementSettings(const std::string& taskFile, bool verbose) const {
  ...
  bool enableCBF = true;
  scalar_t cbfLambda = 1.0;
  RelaxedBarrierPenalty::Config cbfPenaltyConfig(1e-2, 1e-4);
  RelaxedBarrierPenalty::Config statePenaltyConfig(1e-2, 1e-4);
  ...
  loadData::loadPtreeValue(pt, enableCBF, prefix + "enableCBF", verbose);
  loadData::loadPtreeValue(pt, cbfLambda, prefix + "cbfLambda", verbose);
  loadData::loadPtreeValue(pt, cbfPenaltyConfig.mu, prefix + "cbfPenaltyMuParam", verbose);
  loadData::loadPtreeValue(pt, cbfPenaltyConfig.delta, prefix + "cbfPenaltyDeltaParam", verbose);
  loadData::loadPtreeValue(pt, statePenaltyConfig.mu, prefix + "statePenaltyMuParam", verbose);
  loadData::loadPtreeValue(pt, statePenaltyConfig.delta, prefix + "statePenaltyDeltaParam", verbose);
  if (verbose) {
    std::cerr << " #### ================================================================\n";
  }

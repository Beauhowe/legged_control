 ...
 public:
  ...
  TargetTrajectoriesPublisher(rclcpp::Node::SharedPtr node, const std::string& topicPrefix, CmdToTargetTrajectories goalToTargetTrajectories,
      ...
      : node_(std::move(node)),
        ...
        tf2_(buffer_, node_, false) {
    // Trajectories publisher
    targetTrajectoriesPublisher_.reset(new TargetTrajectoriesRosPublisher(node_, topicPrefix));
    ...
    observationSub_ = node_->create_subscription<ocs2_msgs::msg::MpcObservation>(topicPrefix + "_mpc_observation", 1, observationCallback);
    ...
    auto goalCallback = [this](const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg) {
      ...
      try {
        ...
        RCLCPP_WARN(node_->get_logger(), "Failure %s", ex.what());
        return;
      }
    ...
    goalSub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>("/move_base_simple/goal", 1, goalCallback);
    cmdVelSub_ = node_->create_subscription<geometry_msgs::msg::Twist>("/cmd_vel", 1, cmdVelCallback);
  }

...
void FootPlacementVisualization::update(const SystemObservation& observation) {
  if (observation.time - lastTime_ > minPublishTimeDifference_) {
    ...
    for (int leg = 0; leg < numFoot_; ++leg) {
      ...
      int kStart = 0;
      for (int k = 0; k < static_cast<int>(middleTimes.size()); ++k) {
        ...
        auto projectionMarker = getArrowAtPointMsg(projection.regionPtr->transformPlaneToWorld.linear() * vector3_t(0, 0, 0.1),
                                                   projection.positionInWorld, color);
        projectionMarker.header = header;
        projectionMarker.ns = "Projections";
        projectionMarker.id = i;
        projectionMarker.color.a = alpha;
        markerArray.markers.push_back(projectionMarker);

        const auto convexRegion = convexRegionSelector_.getConvexPolygon(leg, middleTimes[k]);
        markerArray.markers.push_back(to3dRosMarker(convexRegion, projection.regionPtr->transformPlaneToWorld, header, color, alpha, i));
...
visualization_msgs::msg::Marker FootPlacementVisualization::to3dRosMarker(
    ...
    const std_msgs::msg::Header& header, Color color, float alpha, size_t i) {

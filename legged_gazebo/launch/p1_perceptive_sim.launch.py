...
def launch_setup(context, *args, **kwargs):
    ...
    launch_dir = os.path.join(controllers_share, "launch")
        ...
        yaml.safe_dump(
            {
                "controller_manager": {
                    "ros__parameters": {
                        "legged_perceptive_controller": {
                            "type": "legged/PerceptiveController",
                        },
                    },
                },
                "legged_perceptive_controller": {
    ...
    spawn_entity = Node(
        ...
        arguments=["-topic", "robot_description", "-entity", f"{robot_type}_perceptive", "-z", "0.5", "-timeout", "180"],
        output="screen",
    )
    ...
    elevation_mapping = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                ...
                "launch",
                "fastlio_elevation_mapping.launch.py",
            )
        ),
        launch_arguments={
            "use_sim_time": LaunchConfiguration("use_sim_time"),
        }.items(),
        condition=IfCondition(LaunchConfiguration("start_elevation_mapping")),
    )
    ...
    convex_plane_decomposition = IncludeLaunchDescription(
        ...
        condition=IfCondition(LaunchConfiguration("start_convex_plane_decomposition")),
    )

    spawn_controllers = RegisterEventHandler(
        OnProcessExit(
            target_action=spawn_entity,
            on_exit=[joint_state_spawner, perceptive_controller_spawner],
        )
    )
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("start_elevation_mapping", default_value="true"),
            DeclareLaunchArgument("emergency_stop_topic", default_value="/p1_emergency_stop"),
            DeclareLaunchArgument("start_convex_plane_decomposition", default_value="true"),
